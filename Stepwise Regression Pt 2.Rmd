...
Here we are transforming back to the original scale, normalizing, then summing.

```{r}
data_merge_proteome <- data_merge_proteome |> 
  mutate(sum_c_pfas = as.numeric(scale(2^(hs_pfhxs_c_Log2))) + 
                       as.numeric(scale(2^(hs_pfna_c_Log2))) + 
                       as.numeric(scale(2^(hs_pfoa_c_Log2))) + 
                       as.numeric(scale(2^(hs_pfos_c_Log2))) + 
                       as.numeric(scale(2^(hs_pfunda_c_Log2))))
```

### Quick side bar
Let's go ahead and do this for the mothers data and make a combined linear reg model.

```{r}
data_merge_proteome <- data_merge_proteome |> 
  mutate(sum_m_pfas = as.numeric(scale(2^(hs_pfhxs_m_Log2))) + 
                       as.numeric(scale(2^(hs_pfna_m_Log2))) + 
                       as.numeric(scale(2^(hs_pfoa_m_Log2))) + 
                       as.numeric(scale(2^(hs_pfos_m_Log2))) + 
                       as.numeric(scale(2^(hs_pfunda_m_Log2))))
```

```{r}
m_c_pfas_sum<-lm(sum_c_pfas~sum_m_pfas, data=data_merge_proteome)
summary(m_c_pfas_sum)
```

### Now back to step two
